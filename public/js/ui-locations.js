...
export function renderLocations() {
    ...
    // Load POI counts async — one parallel request per location
    profile.locations.forEach(loc => {
        fetchApi(`/poi/count?locationId=${encodeURIComponent(loc.id)}`, 'GET')
            .then(data => {
                const el = document.getElementById(`poi-count-${loc.id}`);
                if (el) el.textContent = `POIs: ${data.count}`;
            })
            .catch(() => {});
    });
